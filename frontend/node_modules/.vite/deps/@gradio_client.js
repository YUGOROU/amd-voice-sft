...
async function ft(e) {
	...
	if (s["Content-Type"] = "application/json", typeof window < "u" && window.gradio_config && location.origin !== "http://localhost:9876") {
		if (window.gradio_config.current_page && (e = e.substring(0, e.lastIndexOf("/"))), window.gradio_config.dev_mode || typeof window < "u" && window?.BUILD_MODE === "dev") {
			...
			const o = await be(await this.fetch(t, {
				headers: s,
				credentials: "include"
			}), !!this.options.auth);
...
async function be(e, s) {
	if (e?.status === 401 && !s) {
		...
		throw new Error(n || "Login credentials are required to access this space.");
	} else if (e?.status === 401 && s) throw new Error(De);
	if (e?.status === 200) {
		let t = await e.json();
		return t.dependencies?.forEach((n, o) => {
			n.id === void 0 && (n.id = o);
		}), t;
	} else if (e?.status === 401) throw new Error(ut);
	throw new Error(C);
}
async function _t() {
	const { http_protocol: e, host: s } = await pe(this.app_reference, this.options.token);
	try {
		if (this.options.auth) {
			const t = await Le(e, s, this.options.auth, this.fetch, this.options.token);
...
async function pe(e, s) {
	...
	if (le.test(n)) try {
		const r = (await (await fetch(`https://huggingface.co/api/spaces/${n}/${Ke}`, { headers: t })).json()).host;
		return {
			space_id: e,
			...re(r)
		};
...
function mt(e, s, t) {
	...
	return Object.keys(e).forEach((o) => {
		(o === "named_endpoints" || o === "unnamed_endpoints") && (n[o] = {}, Object.entries(e[o]).forEach(([r, { parameters: i, returns: a }]) => {
			const u = s.dependencies.find((c) => c.api_name === r || c.api_name === r.replace("/", ""))?.id || t[r.replace("/", "")] || -1, l = u !== -1 ? s.dependencies.find((c) => c.id == u)?.types : {
				generator: !1,
				cancel: !1
			};
			if (u !== -1 && s.dependencies.find((c) => c.id == u)?.inputs?.length !== i.length) {
				const c = s.dependencies.find((g) => g.id == u).inputs.map((g) => s.components.find((w) => w.id === g)?.type);
				try {
					c.forEach((g, w) => {
						if (g === "state") i.splice(w, 0, {
							component: "state",
							example: null,
			...
			n[o][r] = {
				parameters: i.map((c) => p(c, c?.component, c?.serializer, "parameter")),
...
var bt = (e = [], s) => {
	...
	return t.forEach((r, i) => {
		if (e.hasOwnProperty(r.parameter_name)) n[i] = e[r.parameter_name];
		else if (r.parameter_has_default) n[i] = r.parameter_default;
		else throw new Error(`No value provided for required parameter: ${r.parameter_name}`);
	}), o.forEach((r) => {
		if (!t.some((i) => i.parameter_name === r)) throw new Error(`Parameter \`${r}\` is not a valid keyword argument. Please refer to the API for usage.`);
	}), n.forEach((r, i) => {
		if (r === void 0 && !t[i].parameter_has_default) throw new Error(`No value provided for required parameter: ${t[i].parameter_name}`);
	}), n;
};
async function vt() {
	...
	if (e && (t.Authorization = `Bearer ${e}`), !!s) try {
		let n, o;
		if (typeof window < "u" && window.gradio_api_info) o = window.gradio_api_info;
		else {
			const r = ae(s.root, this.api_prefix, Qe);
			if (n = await this.fetch(r, {
				headers: t,
				credentials: "include"
			}), !n.ok) throw new Error(L);
			o = await n.json();
		}
...
async function Ot(e, s, t, n) {
	let o = (Array.isArray(e) ? e : [e]).map((i) => i.blob);
	const r = o.filter((i) => i.size > (n ?? Infinity));
	if (r.length) throw new Error(`File(s) exceed the maximum allowed size of ${Rt(n || Infinity)}: ${r.map((i) => `"${i.name}"`).join(", ")}`);
	return await Promise.all(await this.upload_files(s, o, t).then(async (i) => {
...
var Q = class {
	...
	constructor({ path: s, url: t, orig_name: n, size: o, blob: r, is_stream: i, mime_type: a, alt_text: u, b64: l }) {
...
var Tt = typeof process < "u" && process.versions && process.versions.node;
function Se(e, s, t) {
	for (; t.length > 1;) {
		const o = t.shift();
		if (typeof o == "string" || typeof o == "number") e = e[o];
		else throw new Error("Invalid key type");
	}
	const n = t.shift();
	if (typeof n == "string" || typeof n == "number") e[n] = s;
	else throw new Error("Invalid key type");
}
async function ce(e, s = void 0, t = [], n = !1, o = void 0) {
	if (Array.isArray(e)) {
		let r = [];
		return await Promise.all(e.map(async (i, a) => {
			let u = t.slice();
			u.push(String(a));
			const l = await ce(e[a], n ? o?.parameters[a]?.component || void 0 : s, u, !1, o);
			r = r.concat(l);
		})), r;
	} else {
		...
		if (typeof e == "object" && e !== null) {
			...
			for (const i of Object.keys(e)) {
				const a = [...t, i], u = e[i];
				r = r.concat(await ce(u, void 0, a, !1, o));
			}
			return r;
		}
	}
...
function os(e) {
	if (typeof e == "string") {
		...
		if (Tt) return new Pe("upload_file", {
			path: e,
			name: e,
			orig_path: e
		});
...
function V(e, s, t, n, o = !1) {
	if (n === "input" && !o) throw new Error("Invalid code path. Cannot skip state inputs for input.");
	if (n === "output" && o) return e;
	let r = [], i = 0;
	const a = n === "input" ? s.inputs : s.outputs;
	for (let u = 0; u < a.length; u++) {
		const l = a[u];
		if (t.find((c) => c.id === l)?.type === "state") {
			if (o) if (e.length === a.length) {
				const c = e[i];
				r.push(c), i++;
			} else r.push(null);
			else {
...
async function Dt(e, s, t) {
	const n = this;
	await Ct(n, s);
	const o = await ce(s, void 0, [], !0, t);
	return (await Promise.all(o.map(async ({ path: i, blob: a, type: u }) => {
		if (!a) return {
			path: i,
			type: u
		};
...
async function je(e, s, t = []) {
	for (const n in s) s[n] instanceof Pe ? await Lt(e, s, n) : typeof s[n] == "object" && s[n] !== null && await je(e, s[n], [...t, n]);
}
async function Lt(e, s, t) {
	let n = s[t];
	const o = e.config?.root || e.config?.root_url;
	if (!o) throw new Error(Ce);
	try {
		let r, i;
		if (typeof process < "u" && process.versions && process.versions.node) {
			const p = await import("./__vite-browser-external-DYxpcVy9-BSzOQRo0.js");
			i = (await import("./__vite-browser-external-DYxpcVy9-BSzOQRo0.js")).resolve(process.cwd(), n.meta.path), r = await p.readFile(i);
		} else throw new Error(pt);
		const a = new Blob([r], { type: "application/octet-stream" }), u = await e.upload_files(o, [a]), l = u.files && u.files[0];
		if (l) s[t] = new Q({
			path: l,
...
async function Pt(e, s = {}) {
	...
	if (typeof e == "number") this.config.dependencies.find((o) => o.id == e);
	else {
		const o = e.replace(/^\//, "");
		this.config.dependencies.find((r) => r.id == this.api_map[o]);
	}
	return new Promise(async (o, r) => {
		const i = this.submit(e, s, null, null, !0);
		let a;
		for await (const u of i) u.type === "data" && (n && o(a), t = !0, a = u), u.type === "status" && (u.stage === "error" && r(u), u.stage === "complete" && (n = !0, t && o(a)));
	});
}
async function F(e, s, t) {
	...
	try {
		if (o = await fetch(n), r = o.status, r !== 200) throw new Error();
		o = await o.json();
	} catch {
		t({
			status: "error",
...
var Ie = async (e, s) => {
	...
	return new Promise((r) => {
		F(e, le.test(e) ? "space_name" : "subdomain", (i) => {
...
async function $e(e) {
	try {
		const s = await fetch(`https://huggingface.co/api/spaces/${e}/discussions`, { method: "HEAD" }), t = s.headers.get("x-error-message");
...
async function It(e, s) {
	...
	s && (t.Authorization = `Bearer ${s}`);
	try {
		const n = await fetch(`https://huggingface.co/api/spaces/${e}/${Xe}`, { headers: t });
...
async function Ut(e, s, t) {
	...
	const o = { seconds: s };
	try {
		const r = await fetch(`https://huggingface.co/api/spaces/${e}/${et}`, {
			method: "POST",
			headers: {
...
async function qt(e, s) {
	...
	if (o && !ke.includes(o)) throw new Error(`Invalid hardware type provided. Valid types are: ${ke.map((m) => `"${m}"`).join(",")}.`);
	const { http_protocol: a, host: u } = await pe(e, t);
	let l = null;
	if (i) {
		const m = await Le(a, u, i, fetch);
		m && (l = Ne(m));
	}
	const p = {
		...
		"Content-Type": "application/json",
		...l ? { Cookie: l.join("; ") } : {}
	}, c = (await (await fetch("https://huggingface.co/api/whoami-v2", { headers: p })).json()).name, g = e.split("/")[1], w = { repository: `${c}/${g}` };
	n && (w.private = !0);
	let N;
	try {
		o || (N = await It(e, t));
	...
	w.hardware = o || N || "cpu-basic";
	try {
		const m = await fetch(`https://huggingface.co/api/spaces/${e}/duplicate`, {
			method: "POST",
			headers: p,
		...
		else if (m.status !== 200) throw new Error(m.statusText);
		const H = await m.json();
		return await Ut(`${c}/${g}`, r || 300, t), await Y.connect(zt(H.url), s);
	} catch (m) {
		throw new Error(m);
	}
}
...
var Re = "supports-zerogpu-headers";
var Oe = !1;
function Bt() {
...
async function* Jt(e, s) {
	...
	for (;;) {
		if (s && s.aborted) return o.cancel();
		if (n = await o.read(), n.done) return;
		if (!n.value) {
			r && (yield r), r = void 0;
			continue;
		}
...
async function Zt() {
	let { event_callbacks: e, unclosed_events: s, pending_stream_messages: t, stream_status: n, config: o, jwt: r } = this;
	const i = this;
	if (!o) throw new Error("Could not resolve app config");
	n.open = !0;
	let a = null, u = new URLSearchParams({ session_hash: this.session_hash }).toString(), l = new URL(`${o.root}${this.api_prefix}/${Ae}?${u}`);
	...
	a.onmessage = async function(p) {
		...
		const g = c.event_id;
		if (!g) await Promise.all(Object.keys(e).map((w) => e[w](c)));
		else if (e[g] && o) {
			c.msg === "process_completed" && [
				"sse",
				"sse_v1",
				"sse_v2",
				"sse_v2.1",
				"sse_v3"
			].includes(o.protocol) && s.delete(g);
			let w = e[g];
			typeof window < "u" && typeof document < "u" ? setTimeout(w, 0, c) : w(c);
		} else t[g] || (t[g] = []), t[g].push(c);
...
function Yt(e, s, t, n) {
	...
	switch (t) {
		...
		case "delete":
			...
			break;
		default: throw new Error(`Unknown action: ${t}`);
	}
	return e;
}
function Qt(e, s = {}) {
	...
	return Wt(e, s).then(async (n) => {
		t.readyState = t.OPEN;
		try {
			for await (const o of n) t.onmessage && t.onmessage(o);
...
function Xt(e, s = {}, t, n, o, r) {
	try {
		...
		const { token: g } = this.options, { fetch: w, app_reference: N, config: f, session_hash: m, api_info: H, api_map: z, stream_status: X, pending_stream_messages: ee, pending_diff_streams: te, event_callbacks: se, unclosed_events: qe, post_data: ne, options: J, api_prefix: T } = this, he = r || { "x-gradio-user": "api" }, ze = this;
		if (!H) throw new Error("No API found");
		if (!f) throw new Error("Could not resolve app config");
		let { fn_index: d, endpoint_info: fe, dependency: B } = ts(H, e, z, f), Be = bt(s, fe), j, A = f.protocol ?? "ws";
		if (A === "ws") throw new Error("WebSocket protocol is not supported in this version");
		let I = "";
		...
		const Ge = J?.events?.reduce((h, R) => (h[R] = !0, h), {}) || {};
		async function Me() {
			let h = {}, R = {};
			h = { event_id: v }, R = {
				event_id: v,
		...
		async function ge(h) {
			if (!f) return;
			let R = h.render_id;
			f.components = [...f.components.filter((E) => E.props.rendered_in !== R), ...h.components], f.dependencies = [...f.dependencies.filter((E) => E.rendered_in !== R), ...h.dependencies];
			const Z = f.components.some((E) => E.type === "state"), y = f.dependencies.some((E) => E.targets.some((U) => U[1] === "unload"));
		...
		const He = this.handle_blob(f.root, Be, fe).then(async (h) => {
			if (W = {
				data: V(h, B, f.components, "input", !0) || [],
				event_data: t,
				fn_index: d,
				trigger_id: n
			}, At(d, f)) i({
			...
			}), ne(`${f.root}${T}/run${_.startsWith("/") ? _ : `/${_}`}${G ? "?" + G : ""}`, {
				...W,
				session_hash: m
			}, he).then(async ([y, E]) => {
				const U = y.data;
				if (E == 200) i({
					...
					endpoint: _,
					fn_index: d,
					data: V(U, B, f.components, "output", J.with_null_state),
					time: /* @__PURE__ */ new Date(),
					event_data: t,
					trigger_id: n
				...
				let y = new URL(`${f.root}${T}/${Ae}?${G ? G + "&" : ""}${Z}`);
				if (this.jwt && y.searchParams.set("__sign", this.jwt), j = this.stream(y), !j) return Promise.reject(/* @__PURE__ */ new Error("Cannot connect to SSE endpoint: " + y.toString()));
				j.onmessage = async function(E) {
					const { type: O, status: D, data: b } = ve(JSON.parse(E.data), _e[d]);
					if (O === "update" && D && !x) i({
						type: "status",
						endpoint: _,
						fn_index: d,
						...
						let [q, P] = await ne(`${f.root}${T}/queue/data`, {
							...W,
							session_hash: m,
							event_id: v
						});
					...
					b && (i({
						type: "data",
						time: /* @__PURE__ */ new Date(),
						data: V(b.data, B, f.components, "output", J.with_null_state),
						endpoint: _,
						fn_index: d,
						event_data: t,
						trigger_id: n
				...
				return (typeof window < "u" && typeof document < "u" && window.parent != window && !!E && window.supports_zerogpu_headers ? xt("zerogpu-headers", E) : Promise.resolve(null)).then((b) => {
					...
					return ne(`${f.root}${T}/${Ve}?${G}`, {
						...W,
						session_hash: m
					}, q);
				}).then(async ([b, q]) => {
					if (b.event_id && (I = b.event_id), q === 503) i({
						type: "status",
						stage: "error",
						message: xe,
						queue: !0,
						...
						let P = async function(ie) {
							try {
								const { type: S, status: $, data: k, original_msg: We } = ve(ie, _e[d]);
								...
								if (S === "update" && $ && !x) i({
									...
									endpoint: _,
									fn_index: d
								}), k && B.connection !== "stream" && [
									"sse_v2",
									"sse_v2.1",
									"sse_v3"
								].includes(A) && Kt(te, v, k));
								k && (i({
									type: "data",
									time: /* @__PURE__ */ new Date(),
									data: V(k.data, B, f.components, "output", J.with_null_state),
									endpoint: _,
									fn_index: d
								...
								console.error("Unexpected client exception", S), i({
									...
									"sse_v2.1",
									"sse_v3"
								].includes(A) && (de(X, ze.abort_controller), X.open = !1, a());
							}
						};
		...
		const oe = [], M = [], me = {
			...
			close_stream: () => {
				this.post_data(`${f.root}${T}/stream/${I}/close`, {}), a();
			},
			event_id: () => I,
...
function ts(e, s, t, n) {
	let o, r, i;
	if (typeof s == "number") o = s, r = e.unnamed_endpoints[o], i = n.dependencies.find((a) => a.id == s);
	else {
		const a = s.replace(/^\//, "");
		o = t[a], r = e.named_endpoints[s.trim()], i = n.dependencies.find((u) => u.id == t[a]);
	}
	if (typeof o != "number") throw new Error("There is no endpoint matching that name of fn_index matching that number.");
	return {
		fn_index: o,
		endpoint_info: r,
		dependency: i
...
var Y = class {
	...
	get_url_config(s = null) {
		if (!this.config) throw new Error(C);
	...
	async init() {
		Gt(), this.options.auth && await this.resolve_cookies(), await this._resolve_config().then(({ config: s }) => this._resolve_heartbeat(s)), this.api_info = await this.view_api(), this.api_map = ht(this.config?.dependencies || []);
	}
	async _resolve_heartbeat(s) {
		if (s && (this.config = s, this.api_prefix = s.api_prefix || "", this.config && this.config.connect_heartbeat && this.config.space_id && this.options.token && (this.jwt = await ye(this.config.space_id, this.options.token, this.cookies))), s.space_id && this.options.token && (this.jwt = await ye(s.space_id, this.options.token)), this.config && this.config.connect_heartbeat) {
			const t = new URL(`${this.config.root}${this.api_prefix}/${tt}/${this.session_hash}`);
			this.jwt && t.searchParams.set("__sign", this.jwt), this.heartbeat_event || (this.heartbeat_event = this.stream(t));
		}
	}
	static async connect(s, t = { events: ["data"] }) {
		const n = new this(s, t);
		return t.session_hash && (n.session_hash = t.session_hash), await n.init(), n;
	}
	async reconnect() {
		const s = new URL(`${this.config.root}${this.api_prefix}/${it}`);
		let t;
		try {
	...
	static async duplicate(s, t = { events: ["data"] }) {
		return qt(s, t);
	}
	async _resolve_config() {
		const { http_protocol: s, host: t, space_id: n } = await pe(this.app_reference, this.options.token), { status_callback: o } = this.options;
		n && o && await Ie(n, o);
		let r;
		try {
			let i = `${s}//${t}`;
			if (r = await this.resolve_config(i), !r) throw new Error(C);
			return this.config_success(r);
		} catch (i) {
			if (n && o) F(n, le.test(n) ? "space_name" : "subdomain", this.handle_space_success);
			else throw o && o({
				status: "error",
	...
	async handle_space_success(s) {
		if (!this) throw new Error(C);
		const { status_callback: t } = this.options;
		if (t && t(s), s.status === "running") try {
			if (this.config = await this._resolve_config(), this.api_prefix = this?.config?.api_prefix || "", !this.config) throw new Error(C);
	...
	async component_server(s, t, n) {
		...
		let a, u = this.config.components.find((p) => p.id === s);
		u?.props?.root_url ? a = u.props.root_url : a = this.config.root;
		let l;
		if (typeof n == "object" && n !== null && "binary" in n) {
			const p = n;
			l = new FormData();
			for (const c in p.data) c !== "binary" && l.append(c, p.data[c]);
		...
		try {
			const p = await this.fetch(`${a}${this.api_prefix}/${st}/`, {
				method: "POST",
				body: l,
				headers: o,
				credentials: "include"
...
async function is(e, s = { events: ["data"] }) {
...
//#endregion
export { Y as Client, Q as FileData, lt as MISSING_CREDENTIALS_MSG, is as client, rs as duplicate, os as handle_file, Pt as predict, ns as prepare_files, Xt as submit, Ot as upload, Et as upload_files };
