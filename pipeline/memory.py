...
Each session is stored with:
  - patient_id, timestamp, facts (list), emotional_state, confusion_level, summary
...
def upsert_fact(patient_id: str, fact: str) -> None:
    """Add a single fact to the patient's persistent fact list."""
    col = _get_collection()
    facts = get_all_facts(patient_id)
    ...
    col.upsert(
        ...
        ids=[f"{patient_id}_fact_{len(facts)}"],
    )
...
def get_context(patient_id: str, n_sessions: int = 3) -> dict:
    ...
    results = col.query(
        ...
        n_results=n_sessions + 20,  # over-fetch to separate facts from summaries
    )
    ...
    return {
        "summaries": summaries[:n_sessions],
        "facts":     deduped_facts,
    }


def get_all_facts(patient_id: str) -> list[str]:
    return get_context(patient_id)["facts"]
