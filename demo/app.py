...
MODEL_NAME    = os.getenv("MODEL_NAME", "YUGOROU/lumi-qwen3-4b")
...
def text_chat(message: str, history: list[dict]):
    ...
    for token in _call_llm_stream(messages):
        partial += token
        new_history[-1]["content"] = parse_structured_output(partial)["full_response"]
        yield new_history
...
def voice_chat(audio_path: str | None, history: list[dict]):
    ...
    if is_scam:
        ...
        return new_history, audio_out, f"⏱ {time.time()-t0:.1f}s"
    ...
    return new_history, parsed["audio_path"], f"⏱ {latency:.1f}s"
...
with gr.Blocks(title="Lumi — Voice Companion", theme=gr.themes.Soft(), css=CSS) as demo:
    ...
    with gr.Row(elem_id="lumi-header"):
        gr.Markdown("# Lumi — AI Voice Companion\n*Powered by AMD MI300X · Fine-tuned Qwen3-4B*")
    ...
    with gr.Tabs():
        ...
        with gr.Tab("💬 Text Chat"):
            ...
            send1.click(text_chat, [msg1, chatbot1], [chatbot1]).then(
                lambda: "", None, msg1
            )
            msg1.submit(text_chat, [msg1, chatbot1], [chatbot1]).then(
                lambda: "", None, msg1
            )
        ...
        with gr.Tab("🎤 Voice Chat"):
            ...
            audio_in.stop_recording(
                voice_chat,
                [audio_in, chatbot2],
                [chatbot2, audio_out, latency_badge],
            )
...
- **Domain fine-tuned** — Qwen3-4B fine-tuned on 8,500+ dementia-care conversations via EQ-Matrix pipeline
